...
export default function Chapter2() {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-12">
      ...
      {/* 標題 */}
      <div className="bg-gradient-to-br from-amber-700 via-orange-800 to-red-900 rounded-3xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="relative">
          ...
          <h1 className="font-extrabold mb-3" style={{fontSize:"32px"}}>第二單元：生命的意義和價值</h1>
          <p className="text-white/80 text-lg">檢視靈性平安的第一個面向</p>
        </div>
      </div>
        ...
        <AnimatePresence>{showPdf && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
            <div className="mt-4 rounded-xl overflow-hidden border border-amber-200" style={{height:"80vh"}}>
              <iframe src="/spiritual-care.pdf#page=14" className="w-full h-full" title="原書 PDF" />
            </div>
          </motion.div>
        )}</AnimatePresence>
      </div>
