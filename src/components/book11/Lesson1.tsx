...
const GenesisChapter1: React.FC = () => {
  // 定義哪些節是一天的結尾
  const dayEndings = [5, 8, 13, 19, 23, 31];

  return (
    <div className="bg-green-50 border-l-4 border-green-500 p-4 rounded mb-6">
      <p className="text-green-900 font-semibold">神的創造 (創世記第一章)</p>
      <ol className="list-decimal list-inside text-green-900 mt-2">
        {genesis1.map((verse, index) => (
          <div key={index}>
            <li>{verse}</li>
            {dayEndings.includes(index + 1) && (
              <hr className="border-t-2 border-green-300 my-2" />
            )}
          </div>
        ))}
      </ol>
      <p className="text-sm text-green-700 mt-2">
        聖經是「神啟示，人所寫」的證據，而不是摩西能憑自己想像寫的
      </p>
    </div>
  );
};
